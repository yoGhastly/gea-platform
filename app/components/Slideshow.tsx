    ...
    <div className="relative w-full min-h-[700px] rounded-2xl overflow-hidden">
      {posts.map((post, index) => (
        <img
          ...
          className={`w-full h-full object-none absolute top-0 left-0 transition-opacity duration-500 ${index === currentSlide
      ...
      <div className="absolute inset-0 flex flex-col items-start justify-between ml-24">
        ...
        >
          {posts[currentSlide].date.match(/(\d{2}_\d{2}_\d{4})/)![0].replace(/_/g, " ")}
        </Chip>

        <div className="flex gap-10 flex-col basis-3/5 text-white">
          <p className="text-4xl">GEITS</p>
          <p className="text-6xl font-extrabold max-w-md leading-normal">
        ...
        <div>
          {posts.map((_, index) => (
            <button
              ...
              className={`h-4 w-4 rounded-full mx-2 focus:outline-none self-center transition-opacity duration-300 ${index === currentSlide
      ...
      <Link
        ...
        className="absolute bottom-8 right-8 w-12 h-12 bg-white rounded-full flex justify-center items-center"
      >
        <Image
